function [ET,actEvapCanopy,actEvapGrnd,actTranspir,layeredTrans]=canopy_ET(feedback,dt,elevation,...
    Wim,Wi,SSO,root,Wcr,Wwp,W,ice,...% state variables
    airDens,press,rainfall,Tair,Tveg,eActAir,VPD,LAI,netRad,netShort,RaC,...% forcing
...
global EPS RHO_W SECONDS_PER_DAY
nCells=size(W,1);
nLayers=size(W,2);
...
esVeg = saturatedVaporPressure(Tveg);
% -mm/s
basicRate= airDens.*(EPS./press).*(eActAir - esVeg)./RaC/RHO_W;
isEvaporating=basicRate<0;
% when dew forms
actEvapCanopy(~isEvaporating)=-basicRate(~isEvaporating);
...
% gsm_inv=nan(nCells,1);
% gsm_inv(W>Wcr)=1;
% gsm_inv(W<Wwp)=0;
% indexLinear=(W<=Wcr)&(W>=Wwp);
indexZeroIntCap=Wim==0;
indexZeroLAI=LAI==0;
% gsm_inv(indexLinear)=(W(indexLinear) - Wwp(indexLinear))./(Wcr(indexLinear) - Wwp(indexLinear));
% rc=calc_rc(r0c,RGL,netShort,Tair,VPD,LAI,gsm_inv,false);
%%% $$ E_p(0), E_p(r_c)$
rPotEvapCanopy=Penman(elevation,Tair,netRad,VPD,RaC,0,rarc);
%% if the actual ET given by the modified Penman Equation is negative while the direction is evaporation, 
   % which means large negative radiation is incident, use the basic rate
   % to replace the rActEvap and rPotEvapCanopy
evapUnderNegNetRad=isEvaporating&rPotEvapCanopy<0;
% rActEvap(evapUnderNegNetRad)=-basicRate(evapUnderNegNetRad);
rPotEvapCanopy(evapUnderNegNetRad)=-basicRate(evapUnderNegNetRad);
